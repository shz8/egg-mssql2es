...
class mssql2es {
    ...
    async updaterunning(d){
        console.log(d)
        if(d && d.key){
            if(this.rules[d.key]){
                this.rules[d.key].isrunning = d.isrunning;
            }
        }
    }
    async sendtoapp(d){
        this.app.messenger.sendToApp('updaterunning',d );
    }
    async processTask(rule, k) {
        ...
        try {
            ...
            await this.sendtoapp({key:k,isrunning:true})
        ...
        await this.sendtoapp({key:k,isrunning:false})
    }
    async iM(d,esclient) {
        try {
