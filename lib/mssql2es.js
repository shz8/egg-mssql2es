...
class mssql2es {
    ...
    async processTask(rule, k) {
        ...
        try {
            ...
            if (rule.input && rule.output && rule.esclient) {
                ...
                let sql = `select top ${config.size || 1000} ${config.fields || '*'} from ${config.table} `;
                if (max) sql += ` where ${config.order}>'${max}' `
                ...
                if (data && data.length > 0) {
                    ...
                    for (let idx = len - 1; idx >= 0; idx--) {
                        ...
                        let id = `${data[idx][pkey]}`;
                        if (index._id && index._id.indexOf('<') > -1 && index._id.indexOf('>') > -1) {
                            id = ejs.render(index._id, data[idx]);
                        }
                        //this.logger.info(id)
                        data.splice(idx, 0, { index: { _index: index._index, _type: index._type, _id: id } })
                    }
    ...
    async iM(d, esclient) {
        try {
            ...
            if(rlt && rlt.errors){
                let err = 'bulk error!'
                if(rlt.items.length > 0){
                    err = rlt.items[0].index
                }
                this.logger.error(err)
            }
        }
